...
# Copiar arquivos NetCDF para o volume se ainda não existirem
if [ ! -d "/geoserver_data/data/netcdf_samples" ] || [ ! "$(ls -A /geoserver_data/data/netcdf_samples)" ]; then
    ...
    echo "=== Arquivos NetCDF copiados para /geoserver_data/data/netcdf_samples/ ==="
    ls -la /geoserver_data/data/netcdf_samples/
else
    echo "=== Diretório netcdf_samples já existe e contém arquivos ==="
    ls -la /geoserver_data/data/netcdf_samples/
fi

# Copiar estilos SLD para o volume se ainda não existirem
if [ ! -d "/geoserver_data/data/styles" ] || [ ! "$(ls -A /geoserver_data/data/styles)" ]; then
    echo "=== Copiando estilos SLD para o volume ==="
    mkdir -p /geoserver_data/data/styles/
    cp -r /tmp/styles/* /geoserver_data/data/styles/
    echo "=== Estilos SLD copiados para /geoserver_data/data/styles/ ==="
    ls -la /geoserver_data/data/styles/
else
    echo "=== Diretório styles já existe e contém arquivos ==="
    ls -la /geoserver_data/data/styles/
fi
...
then
    ...
    echo "GEOSERVER_LB_PORT is either not defined or empty setting the value to '8080' "
...
# Executar script de registro de estilos em background
echo "=== Iniciando script de registro de estilos em background ==="
nohup sh -c "/usr/local/bin/register_style.sh" &

# Executar script de monitoramento de novas camadas em background
echo "=== Iniciando monitoramento de novas camadas em background ==="
nohup sh -c "/usr/local/bin/monitor_new_layers.sh" &
