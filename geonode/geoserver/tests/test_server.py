...
class LayerTests(GeoNodeBaseTestSupport):
    ...
    def setUp(self):
        ...
        self.UPLOADER_DEFAULT_SETTINGS = {
            ...
            "OPTIONS": {"TIME_ENABLED": False, "MOSAIC_ENABLED": False},
        }
    ...
    def test_style_validity_and_name(self):
        ...
        try:
            ...
            self.assertEqual(san_andres_y_providencia_sld_name, "san_andres_y_providencia_administrative")
            ...
            self.assertEqual(raster_sld_name, "geonode-geonode_gwpollriskafriotest")
    ...
    def test_style_change_on_basic_auth(self):
        ...
        change_style_url = urljoin(
            settings.SITEURL, f"/gs/rest/workspaces/{settings.DEFAULT_WORKSPACE}/styles/{layer.name}?raw=true"
        )
        ...
        authorized = style_change_check(
            post_request, "rest/styles", style_name=f"{layer.name}", access_token=access_token
        )
        self.assertTrue(authorized)

        authorized = style_change_check(
            post_request,
            f"rest/workspaces/{layer.workspace}/styles/{layer.name}",
            style_name=f"{layer.name}",
        ...
        authorized = style_change_check(
            post_request, f"rest/workspaces/{layer.workspace}/layers/{layer.name}", access_token=access_token
        )
        self.assertTrue(authorized)
        ...
        authorized = style_change_check(
            put_request, "rest/styles", style_name=f"{layer.name}", access_token=access_token
        )
        self.assertTrue(authorized)
        ...
        authorized = style_change_check(
            put_request, f"rest/workspaces/{layer.workspace}/layers/{layer.name}", access_token=access_token
        )
        self.assertTrue(authorized)
    ...
    def test_dataset_acls(self):
        ...
        response = self.client.get(reverse("dataset_acls"), **invalid_auth_headers)
    ...
    def test_resolve_user(self):
        ...
        response_json = json.loads(content)
        self.assertEqual(
            {"geoserver": False, "superuser": True, "user": "admin", "fullname": "admin", "email": "ad@m.in"},
            response_json,
        )
    ...
    def test_ogc_server_settings(self):
        ...
        with override_settings(OGC_SERVER=self.OGC_DEFAULT_SETTINGS, UPLOADER=self.UPLOADER_DEFAULT_SETTINGS):
            ...
            OGC_SERVER.update({"PUBLIC_LOCATION": "http://geoserver:8080/geoserver/"})
            ...
            self.assertEqual(ogc_settings.credentials, ("admin", "CFOq3rrLK3mz6oS"))
...
"""
        instance.default_style, _ = Style.objects.get_or_create(
            name=style_name, defaults=dict(sld_title=style_name, sld_body=style_body)
        )
        ...
        # WFS Links
        wfs_links = wfs_links(f"{ogc_settings.public_url}wfs?", instance.alternate, bbox, srid)
        ...
        # WCS Links
        wcs_links = wcs_links(f"{ogc_settings.public_url}wcs?", instance.alternate)
